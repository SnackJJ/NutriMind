...
- [x] **1.3.5b** `get_goal_adherence.py` — 目标达成率分析
  ...
  - 编写单元测试
  - > ⚠️ **[2026-03-13] 合并至 `get_history`** — 200条 trajectory 中仅调用 1 次，teacher 模型无法有效区分两者；adherence 逻辑合并为 `get_history(compare_to_goal=True)` 参数；工具数量 6→5；`src/tools/get_goal_adherence.py` 标记 deprecated（保留文件，供参考）
...
- [x] **1.6.1** 编写集成测试脚本 (`tests/test_smoke.py`)
  ...
  - T1: 目标达成率查询 → `get_history(compare_to_goal=True)` → 回答（原 `get_goal_adherence` 已合并）
  - T2: 多食物计算 → `get_food_nutrition` → `log_meal`
  - T2: 设定目标并查看进度 → ~~`set_goal`~~ → `get_history(compare_to_goal=True)`（`set_goal` 已从 SFT schema 移除）
  - T3: 条件分支 (`get_today_summary` → 预算检查 → `retrieve_knowledge`)
  - T3: 条件分支 (`get_history(compare_to_goal=True)` → 达标率低 → `retrieve_knowledge`)
